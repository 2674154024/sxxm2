  ...
  <view class="page">
    <view class="salary-header">
      <view class="income-card">
        <view class="income-item main">
          <text class="income-label">本月收入</text>
          <text class="income-value highlight">¥{{ summary.monthly_income.toFixed(2) }}</text>
        </view>
        <view class="income-row">
          <view class="income-item">
            <text class="income-label">累计收入</text>
            <text class="income-value">¥{{ summary.total_income.toFixed(2) }}</text>
          </view>
          <view class="income-item">
            <text class="income-label">待结算</text>
            <text class="income-value pending">¥{{ summary.pending_amount.toFixed(2) }}</text>
          </view>
        </view>
      </view>
    </view>
      ...
      <view class="flow-list">
        <view class="flow-card" v-for="record in records" :key="record.record_id" @click="showDetail(record)">
          ...
          <view class="flow-body">
            <text class="work-info">{{ record.work_hours }}小时 × ¥{{ record.hourly_wage }}/小时</text>
          </view>
          <view class="flow-footer">
            <view class="amount-detail">
              <text class="amount-item">应发: ¥{{ record.gross_amount.toFixed(2) }}</text>
              <text class="amount-item">个税: ¥{{ record.tax_amount.toFixed(2) }}</text>
              <text class="amount-item">实发: ¥{{ record.net_amount.toFixed(2) }}</text>
            </view>
            <text class="paid-time" v-if="record.status === 'paid' && record.paid_time">{{ record.paid_time }}</text>
          </view>
        </view>
      </view>
...
.income-item.main .income-value.highlight {
  font-size: 64rpx;
  font-weight: bold;
  color: #FF7D00;
}
...
.income-item .income-value.pending {
  color: #FAAD14;
}
