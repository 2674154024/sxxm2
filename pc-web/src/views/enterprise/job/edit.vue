...
const loadJobDetail = () => {
  ...
  jobApi.getJobDetail(jobId).then(res => {
    ...
    if (data) {
      form.job_name = data.jobTitle || ''
      form.salary = data.salaryAmount ? Number(data.salaryAmount) : 0
      form.salary_type = data.salaryType != null ? convertSalaryType(data.salaryType) : 'hourly'
      settlementType.value = data.settlementType != null ? convertSettlementType(data.settlementType) : 'weekly'
      form.address = data.workAddress || ''
      form.longitude = data.longitude ? Number(data.longitude) : 0
      form.latitude = data.latitude ? Number(data.latitude) : 0
      form.work_time = data.workTime ? JSON.parse(data.workTime) : { weekday: [], time_range: ['', ''] }
      form.skill_tags = data.skillRequire || ''
      form.description = data.description || ''
      form.has_insurance = data.isInsured != null && data.isInsured === 1
      form.recruit_num = data.recruitNum || 1
    }
    loading.value = false
  }).catch((error: any) => {
...
const convertSalaryType = (type: number | null): 'hourly' | 'daily' | 'monthly' => {
...
const convertSettlementType = (type: number | null): 'daily' | 'weekly' | 'monthly' => {
  if (type == null) return 'weekly'
  switch (type) {
    case 0: return 'daily'
...
const submitForm = () => {
  ...
  jobApi.updateJob({
    ...
    const errorMessage = error.response?.data?.message || error.response?.data?.msg || '编辑失败'
...
<template>
  <div class="job-edit" v-loading="loading">
    <div class="page-header">
      <div class="header-left">
        <h1>编辑岗位</h1>
        <p class="subtitle">修改岗位信息，提交审核后即可更新</p>
      </div>
    </div>
